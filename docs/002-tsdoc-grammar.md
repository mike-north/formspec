...
| `@description` | `DescriptionAnnotation` | JSON Schema `description`                | Longer prose; takes precedence over `@remarks` and free-text summary text                                      |
...
| `@remarks`      | TSDoc           | Treated as `@description` when no explicit `@description` is present; wins over free-text summary | JSON Schema `description`                             |
...
**Description precedence:** FormSpec derives a declaration's description using this precedence order:

1. explicit `@description`
2. explicit `@remarks`
3. implicit free-text summary content from the comment block

If both `@description` and `@remarks` are present on the same field, `@description` wins (override rule per C1). `@remarks` is only used as a fallback. Free-text summary content is only used when neither explicit tag is present. A warning (D4-class) is emitted if both `@description` and `@remarks` are present, suggesting the author remove one.
...
Same grammar as `@displayName`. If multiple `@description` tags appear on the same declaration, the last one wins. This follows the general annotation override model (C1). If no `@description` tag is present, FormSpec falls back to `@remarks`, and if neither explicit tag is present, it falls back again to the comment block's free-text summary. Maximum length is configurable (per PP9), independently from `@displayName` — description limits are typically longer.

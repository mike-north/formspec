...
- `@example` documentation values on fields
...
`@example` is a deliberate TSDoc-only exception for v1. It is an ecosystem tag reused as-is (principle S6) and, like the other documentation-oriented ecosystem tags (`@defaultValue`, `@deprecated`, `@remarks`), it has no chain-DSL authoring surface in this revision. It is covered by TSDoc extraction and schema-generation tests (`@example` → JSON Schema `examples`); it does not require a matching ChainDSL option until the spec explicitly adds one.

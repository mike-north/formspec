...
| `@minimum Infinity` | Removed from `KNOWN_DIVERGENCES`                             | Superseded by the #513 follow-up (see callout above): both consumers now **reject** non-finite numerics with `INVALID_NUMERIC_VALUE` and emit no constraint node. No entry needed.                                         |
| `@minimum NaN`      | Removed from `KNOWN_DIVERGENCES`                             | Superseded by the #513 follow-up — same mechanism as `Infinity`. No entry needed.                                                                                                                                          |
...
7. **Phase 5 — delete synthetic callers and narrow-applicability surface.** ✅ Slices A, B, and C all landed. Slice A (snapshot-path Role-B capability guard + placement pre-check) and Slice B (snapshot-path `@const` IR validation) closed the 12 Category-1 / Category-2 canaries. Slice C (this PR) deletes the synthetic surface itself: `runSyntheticProgram`, `buildSyntheticBatchSource`, `buildSyntheticHelperPrelude`, `checkSyntheticTagApplication`, `checkSyntheticTagApplications`, `checkSyntheticTagApplicationsDetailed`, `checkNarrowSyntheticTagApplicability` / `…Applicabilities`, `lowerTagApplicationToSyntheticCall`, the custom `CompilerHost`, the synthetic batch cache, and the supporting-declarations plumbing in `tsdoc-parser.ts` (`buildSupportingDeclarations`, `collectImportedNames`, `rewriteImportedMemberTypes`, `renderSyntheticArgumentExpression`, `pushUniqueCompilerDiagnostics`). Their tests (`tag-capability-applicability.test.ts`, `compiler-signatures.test.ts`, `map-global-synthetic-ts-diagnostics.test.ts`) are removed. Slice C also extends the snapshot Role-B capability guard to cover path-targets — the last remaining reason either consumer needed the synthetic batch. The two `.fails` canaries remaining in `constraint-canaries.test.ts` are Category-3 intentional `@pattern` regression guards, not blockers. Alias-chain divergence (#363) is resolved as a side effect of the unified-path retirement; `KNOWN_DIVERGENCES` is now empty in the parity harness.
...
- **8.4b** Post-Phase 4 gate: peak RSS ≤ **700 MB** on the `stripe-realistic-build` surface (the build consumer driven against the Stripe-realistic fixture). Grounding rationale: the tsserver-plugin Phase 0 baseline is 567 MB — that surface uses the host TypeScript program with zero synthetic `ts.createProgram` calls, so it is the empirically measured cost of loading Stripe's `.d.ts` once. Post-Phase-5 the build surface must do the same work plus schema generation; 700 MB gives 133 MB of headroom above that floor. The current build baseline is 861 MB; the ~294 MB gap corresponds exactly to the synthetic parallel-program cost (consistent with `coldProgramCount: 20` in the synthetic microbenchmark). The previous "≤50% of Phase-0 baseline" target was physically impossible — it would require dropping below the host-checker floor. This gate was grounded in Phase 4 Slice D (see issue #336 comment https://github.com/mike-north/formspec/issues/336#issuecomment-4290907383). **Phase 5C status: ABOVE GATE.** Measured peakRSS_MB warm (median) = **769.5 MB** after synthetic-program deletion (Phase 0 baseline 861.3 MB → Phase 5C 769.5 MB, a 91.8 MB / 10.7% reduction). The synthetic `ts.createProgram` surface is fully retired (verified by the absence of `analysis.syntheticCheckBatch.*` performance events), so the remaining 69.5 MB above the gate lives elsewhere — most likely in host-program caching, TypeScript checker warmup, or a residual supporting-declarations costlet that was not wired into the synthetic allocator. Tracked as a follow-up (not a blocker for Phase 5C itself). The bench reports the gate status at the end of each run but does not exit nonzero when the gate is exceeded — CI dashboards monitor the trend instead.

...
describe("analyzeClass", () => {
  ...
  it("initializes deprecated to false for non-deprecated fields", () => {
    const ctx = createProgramContext(sampleFormsPath);
    const classDecl = findClassByName(ctx.sourceFile, "InstallmentPlan");
    if (!classDecl) throw new Error("InstallmentPlan class not found");
    const analysis = analyzeClass(classDecl, ctx.checker);
    ...
    for (const field of analysis.fields) {
      // eslint-disable-next-line @typescript-eslint/no-deprecated -- testing the `deprecated` flag on FieldInfo
      expect(field.deprecated).toBe(false);
    }
  });

  it("initializes defaultValue to undefined for fields without initializers", () => {
    const ctx = createProgramContext(sampleFormsPath);
    const classDecl = findClassByName(ctx.sourceFile, "InstallmentPlan");
    if (!classDecl) throw new Error("InstallmentPlan class not found");
    const analysis = analyzeClass(classDecl, ctx.checker);

...
describe("generateCodegenOutput", () => {
  ...
  describe("typed accessor generation", () => {
    ...
    it("generates typed accessor function", () => {
      ...
      expect(output).toContain("const types = (UserForm as any).__formspec_types__");
      expect(output).toContain("return { elements: types } as unknown as UserFormFormSpec;");
    });
  });
  ...
  describe("imports and structure", () => {
    it("imports decorated classes from source files", () => {
      const cls = createDecoratedClassInfo({
        name: "TestForm",
        sourcePath: "./test-form",
        typeMetadata: { name: createTypeMetadata({ type: "string" }) },
      ...
      expect(output).toContain('import { TestForm } from "./test-form.js";');
    });

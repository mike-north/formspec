 ...
 *
 * 1. **Build-time (CLI)**: The FormSpec CLI reads decorators through static
 *    analysis of your TypeScript source code.
 *
 * 2. **Runtime (with transformer)**: When used with @formspec/transformer,
 *    type metadata is emitted at compile time, and decorators store their
 *    values. Use `toFormSpec()` to generate specs at runtime.
 *
 * @example
 * ```typescript
 ...
 * }
 *
 * // With @formspec/transformer, get spec at runtime:
...
/**
 * Type metadata emitted by the transformer or codegen.
 *
 * Describes the runtime type information for a field that TypeScript
 * normally erases. Generated by @formspec/transformer at compile time
 * or by `formspec codegen` from static analysis.
 */
 ...
 * This is set by @formspec/transformer at compile time.
 */
 ...
 *
 * **Requirements:**
 * - The class must be compiled with @formspec/transformer to have type metadata
...
export function toFormSpec<T extends new (...args: any[]) => any>(
  ...
  // Get type metadata from transformer (if available)
 ...
 *
 * **Requirements:**
 * - The class must be compiled with @formspec/transformer OR
 * - The class must have had `formspec codegen` run to generate type metadata
 * - Decorators must be applied to store field metadata
 *
 * @param ctor - The class constructor
